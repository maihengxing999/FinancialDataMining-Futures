...
MARGIN_MULTIPLE = 3   # 用于缓解保证金比率带来的高回撤率
...
def handle_bar(context, bar_dict):
    try:
        ...
        for future_M in context.target_list:
            ...
            # 获得该合约的最低保证金比率
            context.margin = instruments(future_M).margin_rate * MARGIN_MULTIPLE
                ...
                try:
                    ...
                    if flag == 0 and position['side'] == 'SELL':
                        if position['quantity'] > 0:
                            ...
                            # 计算最大买开量
                            context.target_nums = int(context.future_account.cash / (context.margin * context.multiplier * bar_dict[future_M].open))
